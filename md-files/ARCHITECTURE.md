...
- **Open/draw leg:** Aqua-mode SwapVM order. Each draw = partial fill. TRUE REVOLVING: the escrow meters outstanding principal against the commitment (repaid draws free capacity — settlements recycle via Aqua `push`, paying the lender while refilling the strategy). The facility leg deliberately carries NO `_stopWhenCovered`: that counter is monotonic and would impose a gross-lifetime turnover ceiling on a revolver; the live Aqua balance (replenished by `push`) is the hard outer bound, the escrow is the sole commitment meter. The cap stays load-bearing on the legs where the maker's counterparty operates the pull (cure/maturity: the borrower's own term sheet; auction: stop selling when the lender is whole). Note: stacks deployed before this change ship the capped facility program (4× commitment gross ceiling) — behaviorally identical until cumulative draws reach 4×. The commitment itself is term-limited (`availabilityEnd`) like a real facility's availability period.
